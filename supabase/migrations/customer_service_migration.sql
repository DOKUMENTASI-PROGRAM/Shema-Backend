-- ============================================
-- Apply Customer Service Migration to Supabase
-- Run this in Supabase SQL Editor
-- ============================================

-- Check if cs schema already exists
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.schemata WHERE schema_name = 'cs') THEN
        RAISE NOTICE 'Creating cs schema...';
    ELSE
        RAISE NOTICE 'cs schema already exists, continuing...';
    END IF;
END $$;
...
DROP TRIGGER IF EXISTS update_sessions_updated_at ON cs.sessions;
...
DROP POLICY IF EXISTS "Anyone can create sessions" ON cs.sessions;
DROP POLICY IF EXISTS "Anyone can read sessions" ON cs.sessions;
DROP POLICY IF EXISTS "Service role can update sessions" ON cs.sessions;
DROP POLICY IF EXISTS "Service role can delete sessions" ON cs.sessions;
DROP POLICY IF EXISTS "Anyone can create messages" ON cs.messages;
DROP POLICY IF EXISTS "Anyone can read messages" ON cs.messages;
DROP POLICY IF EXISTS "Anyone can read admin assignments" ON cs.admin_assignments;
DROP POLICY IF EXISTS "Service role can manage admin assignments" ON cs.admin_assignments;
...
-- SELECT table_name FROM information_schema.tables WHERE table_schema = 'cs';
-- SELECT * FROM cs.sessions LIMIT 1;
-- SELECT * FROM cs.messages LIMIT 1;
-- SELECT * FROM cs.admin_assignments LIMIT 1;
...
DO $$ 
BEGIN
    RAISE NOTICE '✅ Customer Service schema created successfully!';
    RAISE NOTICE 'Tables: cs.sessions, cs.messages, cs.admin_assignments';
